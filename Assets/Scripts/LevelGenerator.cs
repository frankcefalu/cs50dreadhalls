...
public class LevelGenerator : MonoBehaviour {
	...
	void Start () {
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				if(holesCounter <= 4)
                {
					CreateFloorPrefab(floorPrefab, floorParent, x, 0, z);
				}
	...
	void CreateFloorPrefab(GameObject prefab, GameObject parent, int x, int y, int z)
	{
		
		var myPrefab = Instantiate(prefab, new Vector3(x, y, z), Quaternion.identity);
		Vector3 SpawnPos1 = myPrefab.transform.position;
		Vector3 SpawnPos2;
		SpawnPos2.x = SpawnPos1.x;
		SpawnPos2.y = SpawnPos1.y + 1;
		SpawnPos2.z = SpawnPos1.z;
		Collider[] hitColliders = Physics.OverlapCapsule(SpawnPos1, SpawnPos2, .1f);

	

		if (hitColliders.Length == 1) //You have someone with a collider here
		{
